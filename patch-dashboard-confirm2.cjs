...
code = code.replace(/{[\s]*\/\* Modal Footer \*\/[\s]*<div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-between items-center">[\s\S]*?<\/div>/g, guestFooterReplacement);
code = code.replace(/{[\s]*\/\* Modal Footer \*\/[\s]*<div className="p-4 bg-slate-50 border-t border-slate-200 flex justify-between items-center shrink-0">[\s\S]*?<\/div>/g, reportFooterReplacement);
